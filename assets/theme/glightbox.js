...
{
    $.ajax({
        url: "/controller/client/GioHang.php",
        method: "POST",
        dataType: "JSON",
        data: {
            'id'       : id
        ...
        success: function(respone) {
            ...
            $('#GioHang' + id).html('<i class="fa-sharp fa-regular fa-circle-check"></i> Đã thêm').prop('disabled', false);
...
$(document).ready(function() {
    $("form[submit-ajax=ken]").submit(function(e) {
        e.preventDefault();
        let _this = this;
        ...
        if (button.attr("order")) {
            Swal.fire({
                title: "XÄ‚Â¡c nhĂ¡ÂºÂ­n thanh toÄ‚Â¡n!",
                text: button.attr("order"),
                icon: "warning",
                showCancelButton: true,
                confirmButtonColor: "#3085d6",
                cancelButtonColor: "#d33",
                confirmButtonText: "TÄ‚Â´i Ă„â€˜Ă¡Â»â€œng Ä‚Â½",
                cancelButtonText: "Ă„ÂÄ‚Â³ng",
            }).then((result) => {
...
function submitForm(url, method, href, data, button) {
    ...
    let setting = {
        ...
        beforeSend: function() {
            button
                .prop("disabled", !0)
                .html('<i class="fas fa-spinner fa-spin"></i> Ă„Âang xĂ¡Â»Â­ lÄ‚Â½...');
        },
        complete: function() {
        ...
        success: (data) => {
            if (data.error) {
                Swal.fire('ThÄ‚Â´ng bÄ‚Â¡o', data.msg, "error");
            } else {
                Swal.fire('ThÄ‚Â´ng bÄ‚Â¡o', data.msg, "success");
                if (!data.href) {
                    setTimeout(function() {
